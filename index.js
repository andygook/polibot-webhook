...
/**
 * PoliBOT Webhook — index.js (completo)
 *
 * - Mantiene todas las opciones del menú principal (1–5) tal como en tu archivo anterior.
 * - Opción 6 (Contactar asistente académico) mejorada con submenú, términos S/N,
 *   captura de nombre, cédula y celular (10 dígitos) y envío de correo (Nodemailer).
 * - Todo administrado desde el webhook con contextos (sin requerir intents nuevos obligatorios).
 * - CSV de estudiantes cargado desde GitHub al inicio.
 */
...
// (Se mantiene tu token como en el archivo anterior para no romper integración)
// =========================
const TELEGRAM_BOT_TOKEN = '7253134218:AAFVF7q25Ukx24IcGOgw-T3-ohzMYQRN0Lk'; // Token proporcionado

// =========================
// Configuración de correo (Nodemailer)
// - Usa variables de entorno en Render: EMAIL_USER, EMAIL_PASS
// =========================
...
function loadData() {
  return new Promise((resolve, reject) => {
    axios.get('https://raw.githubusercontent.com/andygook/polibot-webhook/main/estudiantes_info.csv')
      .then(response => {
        parse(response.data, { columns: true, skip_empty_lines: true, trim: true }, (err, records) => {
          if (err) {
...
// =========================
// Textos requeridos para Opción 6
// =========================
...
app.post('/', (req, res) => {
  ...
  // ==============
  // Handlers existentes (1–5) — preservados del archivo anterior
  // ==============
  ...
  function mainMenuHandler(agent) {
    ...
    if (emojiRegex.test(input)) {
      ...
      const message = 'Lo siento, no entendí tu solicitud. Por favor, selecciona una opción válida.\n\n' + MAIN_MENU_TEXT;
      agent.add(new Payload(agent.TELERAM, { text: message }));
      // Corrige: si typo en TELEGRAM, enviar manual de todos modos
      agent.add(new Payload(agent.TELEGRAM, { text: message }));
      sendTelegramMessage(chatId, message);
      agent.context.set({ name: 'main_menu', lifespan: 5 });
      return;
    ...
    if (input === '5') {
      ...
      // NUEVO: Submenú solicitado para Opción 6
  ...
  function contactAssistanceHandler(agent) {
    ...
    let input = (agent.query || agent.parameters.option || '').trim().toLowerCase();

    if (!contactAssistanceContext) {
      // Si no hay contexto, no hacer nada especial
    ...
    if (input === '1') {
      ...
      agent.add(new Payload(agent.TELEGRAM, { text: message }));
      sendTelegramMessage(chatId, message);
      // Mantener el contexto para permitir 0 y regresar
      agent.context.set({ name: 'contact_assistance', lifespan: 5 });
      return;
    }
    ...
    // Cualquier otra entrada: reimprime submenú
    const message = 'Opción inválida.\n\n' + AAS_SUBMENU_TEXT;
    agent.add(new Payload(agent.TELEGRAM, { text: message }));
    sendTelegramMessage(chatId, message);
    agent.context.set({ name: 'contact_assistance', lifespan: 5 });
  ...
  async function aasCollectPhoneHandler(agent) {
    ...
    // Completar datos y enviar correo
    const c = agent.context.get('aas_data');
  ...
  // =========================
  // Router por contexto para manejar fallbacks
  // =========================
  function router(agent) {
    // Prioridad: flujos AAS (opción 6)
    ...
    // Menú personalizado (opción 5) y otros contextos
    ...
    // Si no hay contextos, mostrar menú principal
  ...
  // Usamos router para el Default Fallback para respetar contextos activos
  intentMap.set('Default Fallback Intent', router);

  // Intents existentes del archivo anterior
