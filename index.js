...
loadData().then(() => {
    app.post('/', (req, res) => {
        ...
        function welcomeHandler(agent) {
            ...
            const message = `¡Bienvenido(a), soy <strong>PoliBOT<\strong>!, tu asistente virtual en postgrado. ¿Cómo puedo ayudarte?\n\n` +
                            `**Menú Principal:**\n` +
        ...
        function processPersonalizedQueriesHandler(agent) {
            ...
            if (!personalizedQueriesContext || !input) {
                agent.add('Error: No se detectó el contexto o la opción. Digite g para regresar al menú anterior.');
                return;
            }
            ...
            if (!project) {
                ...
                agent.add('Error: No se encontraron datos del proyecto. Digite g para regresar al menú anterior.');
                return;
            }
